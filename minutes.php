...
function parseCase($caseStr) {
	...
	if($remaining != "") {
		...
		if(isset($case['vote'])) {
			$case['status'] = inferStatus($case["vote"]);
		} else if(isset($case['discussion'])) {
			$case['status'] = inferStatus($case["discussion"]);
		} else {
			$case['status'] = 'NOVOTE';
		}
