...
function parseDecisions($mode = 'csv') {
	...
	if($mode == 'csv') {
		...
		$rows = array_map(function($row) {return "<tr onclick=\"search('".@explode(",", $row)[2]."')\"><td>".implode("</td><td>",explode(",", $row))."</td></tr>";}, $rows);
...
<head>
	...
	<script>
		function search(appeal) {
			...
			modal = document.querySelector("#appeal");
			modal.innerHTML = "<pre style=\"white-space: pre-wrap;\">"+JSON.stringify(_case, null, 2)+"</pre>";
			modal.showModal();
...
<h2>Introduction</h2>
<p>This site Boston Zoning Board of Appeal decisions and minutes
...
<p><strong>Disclaimner:</strong> this site is for informational and entertainment purposes only.</p>
...
<dialog id="appeal" style="width:75%"></dialog>
</body>
</html>
